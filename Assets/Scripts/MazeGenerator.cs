...
public class MazeGenerator : MonoBehaviour
{
    ...
    public int enemyCount = 2; // Number of enemies
    private int enemiesSpawned = 0; // Keep track of enemies spawned so far
    ...
    private void MakeMaze()
    {
        ...
        for (int i = 0; i < width; i++)
        {
            ...
            for (int j = 0; j < height; j++)
            {
                ...
                {
                    // Odds of an enemy being spawned on a given tile are slightly greater than (enemyCount / tileCount)
                    float spawnChance = enemyCount / ((float)(width * height - Math.Min(width, height)));
